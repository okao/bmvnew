...
export async function markEmailAsVerified(rawInput: MarkEmailAsVerifiedInput) {
  try {
    const validatedInput = markEmailAsVerifiedSchema.safeParse(rawInput)

    //dummy call to verify email
    await fetch("https://api.minoos.com/api/v1/users/verify-email", {
      method: "POST",
      headers: {
